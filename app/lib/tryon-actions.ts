...
export async function analyzePersonImage(personImageFile: File) {
  ...
  try {
    ...
    return {
      bodyType: extractValue(analysisText, ['slim', 'athletic', 'average', 'curvy', 'plus size'], 'Average'),
      ...
      skinTone: extractValue(analysisText, ['fair', 'medium', 'olive', 'dark'], 'Medium'),
      pose: 'Standing',
    ...
    throw new Error(`Failed to analyze person image: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
export async function analyzeClothingImage(clothingImageFile: File) {
  ...
  try {
    ...
    // Fallback: extract information from text response
    return {
      type: extractValue(analysisText, ['t-shirt', 'dress', 'jacket', 'sweater', 'blouse', 'pants', 'skirt'], 'T-Shirt'),
      primaryColor: extractValue(analysisText, ['black', 'white', 'blue', 'red', 'green', 'pink', 'purple'], 'Blue'),
      secondaryColor: null,
      pattern: 'Solid',
      material: 'Cotton',
      style: 'Casual',
    ...
    throw new Error(`Failed to analyze clothing image: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}

// Generate try-on image using OpenAI image.edit()
export async function generateTryOnWithOpenAI(personImageFile: File, clothingImageFile: File) {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY not configured')
  }
  ...
  try {
    ...
    // Convert person image to buffer for OpenAI
    const personImageBytes = await personImageFile.arrayBuffer()
    const personImageBuffer = Buffer.from(personImageBytes)
    
    // Get clothing image as data URL for analysis
    const clothingImageDataURL = await fileToDataURL(clothingImageFile)
    
    // First, analyze the clothing to create an appropriate prompt
    // Create OpenAI client inside the function
    const { default: OpenAI } = await import('openai')
      ...
      messages: [
        {
          ...
          content: [
            {
              type: "text",
              text: "Look at this clothing item and create a concise prompt for DALL-E image editing. The prompt should describe what clothing to add/change on a person while maintaining their pose, background, and appearance. Focus only on the clothing item and how it should look when worn. Keep it under 100 words."
            },
            {
              type: "image_url",
              image_url: {
    ...
    const editPrompt = clothingAnalysisResponse.choices[0]?.message?.content || 'Add the clothing item to the person'
    
    console.log('Generated edit prompt:', editPrompt)
    
    // Use OpenAI image.edit() to apply the clothing to the person
    // Create OpenAI client inside the function
...
export async function generateTryOnWithFalAI(personImageFile: File, clothingImageFile: File) {
...
export async function generateTryOnImage(formData: FormData) {
  try {
    ...
    for (const img of [...personImages, ...clothingImages]) {
      if (img.size > maxSize) {
        throw new Error(`File "${img.name}" is too large. Maximum size is 10MB.`)
      }
    }
...
function extractValue(text: string, options: string[], defaultValue: string): string {
